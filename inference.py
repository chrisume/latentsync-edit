...
def load_models(config: Any, checkpoint_path: str, device: str, precision: str) -> Dict[str, torch.nn.Module]:
  ...
  try:
      ...
      unet_loaded = False
      for key in ['unet', 'model', 'generator', 'net_g', 'netG', 'G']:
          if key in checkpoint:
              print(f"Loading UNet from key: {key}")
              try:
                  unet.load_state_dict(checkpoint[key])
                  unet_loaded = True
                  break
              except Exception as e:
                  print(f"Error loading UNet from key {key}: {e}")
      
      if not unet_loaded:
          print("WARNING: Could not load UNet weights. Using random initialization.")
      
      # Try to load phoneme mapper weights
      mapper_loaded = False
      for key in ['phoneme_mapper', 'mapper', 'audio_encoder', 'audio_net', 'netA']:
          if key in checkpoint:
              print(f"Loading phoneme mapper from key: {key}")
              try:
                  phoneme_mapper.load_state_dict(checkpoint[key])
                  mapper_loaded = True
                  break
              except Exception as e:
                  print(f"Error loading phoneme mapper from key {key}: {e}")
      
      if not mapper_loaded:
          print("WARNING: Could not load phoneme mapper weights. Using random initialization.")
      
  except Exception as e:
      print(f"Error loading checkpoint: {e}")
      print("WARNING: Using random initialization for all models.")
...
def save_output(
  ...
  print(f"Creating video writer for: {output_path}")
  writer = cv2.VideoWriter(output_path, fourcc, fps, (width, height))
  
  if not writer.isOpened():
      print(f"ERROR: Could not open video writer for {output_path}")
      # Try with a different codec
      print("Trying with a different codec (XVID)...")
      writer = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*"XVID"), fps, (width, height))
      if not writer.isOpened():
          print(f"ERROR: Still could not open video writer. Saving frames as images instead.")
  ...
  # Release writer
  writer.release()
  print(f"Video writer released. Checking if file exists: {os.path.exists(output_path)}")
  ...
  try:
      ...
      print(f"Renaming {output_path} to {temp_output}")
      ...
      print(f"Temporary file removed. Final output: {output_path}")
      print(f"Final file exists: {os.path.exists(output_path)}, Size: {os.path.getsize(output_path) if os.path.exists(output_path) else 'N/A'} bytes")
  except Exception as e:
...
def main():
  ...
  try:
      ...
      print("Falling back to InsightFace detector")
      args.detector = "insightface"
      face_processor = FaceProcessor(
